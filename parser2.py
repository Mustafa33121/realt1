...
def get_last_page() -> int:
    ...
    page = soup.find_all('a',
                         class_='focus:outline-none sm:focus-visible:shadow-10bottom cursor-pointer select-none inline-flex font-normal text-body min-h-[2.5rem] min-w-[2.5rem] py-0 items-center !px-1.25 justify-center mx-1 hover:bg-basic-200 rounded-md disabled:text-basic-500')
    page = page[-1].text
    page = int(page)
    return page
...
def get_flat_data(url: str) -> dict:
    ...
    if resp.ok:
        ...
        try:
            description = soup.find('div', class_=['description_wrapper__tlUQE']).text
        except Exception as e:
            description = ''
        ...
        for param in params:
            ...
            value = param.find(['p', 'a']).text.replace(' м²', '').replace('\xa0', '').replace('г.', '')
...
def run():
    ...
    links = get_flats_links(last_page)
